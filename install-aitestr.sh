...
#  aRepo="$1"; if [ "$1" == "" ]; then aRepo="AIDocs_demo1"; fi                         # .(50402.15.5)
   ...
   aAppDir="$( pwd )"                                                                   # .(50610.04.3 Force it)
...
#  trap    cleanup SIGINT SIGTERM SIGHUP  # Set trap to catch SIGINT                    ##.(50408.03.5).(50610.04.4)

#          frt clone    ${aRepo}  ''  ${aAppDir}  ''  ${aAccount}  -d;   # exit         ##.(50610.04.5 RAM Don't reinstall)
...
# --------------------------------------------------------------
      aOriginalRepoDir=${aRepoDir}; aRepoDir="."                                        # .(50610.04.6)
...
if [ -f "${aRepoDir}/._2/package.json" ]; then                                          # .(50402.20.1 RAM Run other npm installs Beg)
   echo -e "  cd ${aRepoDir}/._2";  # exit                                              # .(50106.06.6).(50104.01.10)
     ...
     cd ../                                                                             # .(50610.04.7 RAM only go up one level)
     fi
#   --------------------------------------------------
...
if [ -f "${aRepoDir}/server1/package.json" ]; then
   ...
   echo "  npm install"
     ...
     cd ../                                                                             # .(50610.04.9).(50406.01c.1 RAM Return back to root dir)
     fi                                                                                 # .(50402.20.1 End)
# --------------------------------------------------------------
...
# --------------------------------------------------------------
                                                              ...
                                                              bModelTester=1            # .(50610.04.10 RAM Why wasn't it set?)
...
#   fi
   echo ""                                                                              # .(50406.01e.1)
#  cpyEnv "./${aRepoDir}/client1/c16_aidocs-review-app/utils/FRTs"  "_env_local-local.txt"  "_env"          ##.(50406.03.2 RAM Copy c16 _env).(50610.04.11)
   cpyEnv   "${aRepoDir}/client1/c16_aidocs-review-app/utils/FRTs"  "_env_local-local.txt"  "_env"          # .(50610.04.11 RAM Remove ".")
 ...
 if [ "${bModelTester}" != "0" ]; then                                                                      # .(50406.03c.1 RAM Try again).(50406.03b.1 RAM Only copy dev01's env)
   cpyEnv   "${aRepoDir}/server/s01_search-app"      ".env_example"  ".env"                                 # .(50610.04.12).(50518.01.7)
   cpyEnv   "${aRepoDir}/server1/s11_search-mod-app" ".env_example"  ".env"                                 # .(50610.04.13).(50518.01.8 RAM Rename).(50410.01.2)
   cpyEnv   "${aRepoDir}/server1/s12_search-web-app" ".env_example"  ".env"                                 # .(50610.04.14).(50410.01.3).(50406.03.3 RAM Copy s12 .env)
   cpyEnv   "${aRepoDir}/server1/s13_search-rag-app" ".env_example"  ".env"                                 # .(50610.04.15).(50410.01.4)
   cpyEnv   "${aRepoDir}/server1/s14_scoring-app"    ".env_example"  ".env"                                 # .(50610.04.16).(50602.02c.1 RAM Change grading to scoring).(50510.05.1)

#  cd ${aRepoDir}                                                                                           # .(50610.04.17 RAM We are already here).(50505.10.1)
...
#  echo "--- aRepo: '${aRepo}', aStage: '${aStage}', bModelTester: '${bModelTester}'"
 ...
 if [ "${bModelTester}" != "0" ]; then                                                  # .(50406.01d.2).(50406.01c.2).(50408.01.x).(50402.15.7 RAM Only set ANYLLM_KEY id it's installed)
   ...
   aRepoDir=${aOriginalRepoDir}                                                         # .(50610.04.18)
...
#  echo -e   "     bash run-tests2.sh"                                                  # .(50410.01.8)
   ...
   echo -e "* AIDocs didn't get installed into folder: ${aRepoDir}.";                   # .(50610.04.19 RAM Remove /client1.(50402.15.9)
#  exit
