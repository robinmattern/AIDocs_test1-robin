...
# -------------------------------------------------------------------
   ...
   if [ "${aCmd}" == "help" ]; then                                                                                        # .(50420.04.1)
      ...
      echo -e   "    ${aAIT}  t0##  # A single test for one sysprompt. (generated from s11_model-tests.txt)"               # .(50505.02.3).(50420.04.2 RAM Add Help re run-tests.sh)
      ...
      echo -e   "    ${aAIT}  t041  # .env generated from .env_${aApp}_t040_qwen2;0.5b_4,6-tests.txt"                      # .(50505.02.6).(50422.03.2 RAM Was s11).(50420.04.5)
      echo -e   "    ${aAIT}  curr  # .env as it currently exists"                                                         # .(50505.02.7).(50430.01.1 RAM Add to help)
...
#     echo "=================================================================================================================================================="
      ...
      if [ "${bDebug}" == "1" ]; then node --inspect-brk ${searchScript}  "$@"          # .(50501.04.1 RAM Add Attach debug).(50423.03.3 RAM Use ${Search_Script} instead of search_u2.05.sh)
                                 else node               ${searchScript}  "$@"; fi      # .(50501.04.2)
