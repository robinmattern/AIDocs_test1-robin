...
function  sayMsg() {                                                                                        # .(50531.05.5 RAM Defined at top)
#  if [ "${bDebug}" == "1" ]; then echo -e "  - $1"; fi
   if [ "${bDebug}" == "1" ]; then echo -e "\x1b[93m  - $1\x1b[0m"; fi                                      # .(50531.05.6 RAM Add yellow)
   }
# -----------------------------------------------------------------

   bDebug=${DEBUG}; if [ "${bDebug}" == "1" ]; then                                                         # .(50503.10.1 RAM Even though sayMsg isn't defined yet)  
            sayMsg ""                                                                                       # .(50531.05.7)
            sayMsg "AIC19[  62]  bDebug: '${bDebug}', aApp2: '${aApp2}', aPcCd: '${aPcCd}', aCollection: '${aCollection}', aEnvFile: '${aEnvFile}'"  # .(50531.05.8 RAM Was echo)
   fi                                                                                                       # .(50503.10.2)
...
#           mArray[$i]=$(echo "${mArray[$i]}" | sed 's/^[[:space:]]*//;s/[[:space:]]*$//')   # '
#           done
            template_file="$1" # Read the template file
            output_file="$2"
