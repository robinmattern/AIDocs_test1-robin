...
#    export LOGGER="log,inputs"
     ...
     bash  "${aRun_Tests}"  t041;                                if [ $? -ne 0 ]; then exit 1; fi
     node  "${aScore_Script}" "gemma2:2b" ;                      if [ $? -ne 0 ]; then exit 1; fi
 
     bash  "${aRun_Tests}"  t042;                                if [ $? -ne 0 ]; then exit 1; fi
     node  "${aScore_Script}" "gemma2:2b" ;                      if [ $? -ne 0 ]; then exit 1; fi
 
     bash  "${aRun_Tests}"  t043;                                if [ $? -ne 0 ]; then exit 1; fi
     node  "${aScore_Script}" "gemma2:2b" ;                      if [ $? -ne 0 ]; then exit 1; fi
 
     bash  "${aRun_Tests}"  t044;                                if [ $? -ne 0 ]; then exit 1; fi
     node  "${aScore_Script}" "gemma2:2b" ;                      if [ $? -ne 0 ]; then exit 1; fi
 
     bash  "${aRun_Tests}"  t045;                                if [ $? -ne 0 ]; then exit 1; fi
     node  "${aScore_Script}" "gemma2:2b" ;                      if [ $? -ne 0 ]; then exit 1; fi
 
     bash  "${aRun_Tests}"  t046;                                if [ $? -ne 0 ]; then exit 1; fi
     node  "${aScore_Script}" "gemma2:2b" ;                      if [ $? -ne 0 ]; then exit 1; fi
 
     bash  "${aRun_Tests}"  t047;                                if [ $? -ne 0 ]; then exit 1; fi
     node  "${aScore_Script}" "gemma2:2b" ;                      if [ $? -ne 0 ]; then exit 1; fi
 
     bash  "${aRun_Tests}"  t048;                                if [ $? -ne 0 ]; then exit 1; fi
     node  "${aScore_Script}" "gemma2:2b" ;                      if [ $? -ne 0 ]; then exit 1; fi
 
     bash  "${aRun_Tests}"  t049;                                if [ $? -ne 0 ]; then exit 1; fi
     node  "${aScore_Script}" "gemma2:2b" ;                      if [ $? -ne 0 ]; then exit 1; fi
